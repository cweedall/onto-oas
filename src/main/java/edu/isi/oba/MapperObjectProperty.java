...
/**
 * Class for generating an new object property {@link Schema} OR taking an existing one and updating
 * it.
 */
public class MapperObjectProperty extends MapperProperty {
	 ...
	 * @param references a {@link Set} of {@link String} references to other objects (i.e. the object
	 *     property range(s)).
	 * @return a generated {@link Schema} for the object property.
	 */
	public static Schema createObjectPropertySchema(
			String name, String description, Set<String> references) {
		...
		// For one item, set items.  For multiple items, set items with an anyOf array with the items
		// array.
	 ...
	 *
	 * @param objectSchema an object {@link Schema}.
	 * @param complementOfReference a {@link String} value (i.e. OWLClass' short form name) to set as
	 *     the complement.
	 */
	public static void setComplementOfForObjectSchema(
	 ...
	 */
	public static void addAnyOfToObjectPropertySchema(
	 ...
	 * @param complexObjRangeSchema a {@link Schema} containing one or more object range references.
	 */
	public static void addAllOfToObjectPropertySchema(
			Schema objectPropertySchema, Schema complexObjRangeSchema) {
		// Always set nullable to true for owl:allValuesFrom
		// @see https://owl-to-oas.readthedocs.io/en/latest/mapping/#allValuesFromExample
		MapperProperty.setNullableValueForPropertySchema(objectPropertySchema, true);
		...
		if (itemsSchema.getEnum() == null || itemsSchema.getEnum().isEmpty()) {
			...
			if (itemsSchema.getAllOf() == null
					|| !itemsSchema.getAllOf().contains(complexObjRangeSchema)) {
				// There are cases where the property has a range (i.e. the items schema has a ref), but
				// class restrictions have been added which further restrict it with an allOf.
				// So, we need to unset the items reference first.
				if (itemsSchema.get$ref() != null) {
	...
	public static void addHasValueOfPropertySchema(
			Schema propertySchema, OWLIndividual hasValueIndividual) {
		// Call super class's method after determining named individaul's short name.
		MapperProperty.addHasValueOfPropertySchema(
				propertySchema, ((OWLNamedIndividual) hasValueIndividual).getIRI().getShortForm());
	}
